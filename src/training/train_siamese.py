"""Phase 5 training loop for the Siamese U-Net. Structurally identical
to train_baseline (same loss, checkpointing, resume logic) — only the
model forward signature differs (two inputs, not one)."""
import torch
...
def train_siamese(train_dataset, val_dataset, checkpoint_dir: str,
                   epochs: int = 20, batch_size: int = 8, lr: float = 1e-3,
                   resume: bool = True):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Device: {device}")

    model = SiameseUNet(in_channels=6).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)

    ckpt_path = Path(checkpoint_dir) / "siamese_latest.pt"
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if agg["f1"] > best_f1:
            ...
            torch.save({"epoch": epoch, "model_state": model.state_dict(), "val_metrics": agg},
                       Path(checkpoint_dir) / "siamese_best.pt")
